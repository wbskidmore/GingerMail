...
function loadDrivers() {
    ...
    let plain;
    try {
        plain = localRequire('better-sqlite3');
    }
    catch {
        plain = undefined;
    }
    let encrypted;
    try {
        encrypted = localRequire('better-sqlite3-multiple-ciphers');
    }
    catch {
        encrypted = undefined;
    }
...
export function openEncryptedDatabase(opts) {
    ...
    if (!encryptionKeyHex) {
        if (!PlainDriver) {
            throw new Error('[storage] no encryptionKeyHex was supplied but `better-sqlite3` is not installed. ' +
    ...
    if (!HEX_KEY_RE.test(encryptionKeyHex)) {
        throw new Error(`[storage] encryptionKeyHex must be 64 hex chars (256 bits); got length ${encryptionKeyHex.length}`);
    }
    if (!EncryptedDriver) {
        ...
        throw new Error('[storage] encryptionKeyHex was supplied but `better-sqlite3-multiple-ciphers` is not installed. ' +
    ...
    const file = createRequire(import.meta.url)('node:fs');
...
function openWith(Driver, dbPath, readonly) {
 ...
 */
function migratePlaintextToEncrypted(drivers, dbPath, encryptionKeyHex) {
...
//# sourceMappingURL=openEncryptedDb.js.map