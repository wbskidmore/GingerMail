...
export class ImapSmtpProvider {
    ...
    async listMessageHeaders(folderId, cursor, limit = 100) {
        await this.ensure();
        ...
        try {
            ...
            if (total === 0 || endSeq < 1)
                return { items: [] };
            const headers = [];
            for await (const msg of this.imap.fetch(`${startSeq}:${endSeq}`, { envelope: true, flags: true, internalDate: true, uid: true, bodyStructure: true })) {
    ...
    async getMessage(folderId, uid) {
        ...
        try {
            ...
            const lu = parseListUnsubscribe(parsed.headers?.['list-unsubscribe'] ?? parsed.headers?.['List-Unsubscribe'], parsed.headers?.['list-unsubscribe-post'] ?? parsed.headers?.['List-Unsubscribe-Post']);
            return {
                ...header,
                body: {
                    html: parsed.html,
    ...
    async setFlag(input) {
        ...
        try {
            ...
            if (op.add)
                await this.imap.messageFlagsAdd(input.uid, op.add, { uid: true });
            if (op.remove)
                await this.imap.messageFlagsRemove(input.uid, op.remove, { uid: true });
        }
        finally {
            lock.release();
        }
    ...
    async moveMessage(input) {
        ...
        try {
            const res = await this.imap.messageMove(input.uid, toPath, { uid: true });
    ...
    async reportSpam(input) {
        ...
        if (!spam)
            throw new Error('No spam folder configured for this account.');
    ...
    async search(query) {
        ...
        const lock = await this.imap.getMailboxLock(path);
        try {
            const uids = await this.imap.search({ or: [{ subject: query }, { body: query }, { from: query }] }, { uid: true });
    ...
    watch(folderId, cb) {
        const onExists = async (data) => {
            if (data.path !== folderIdToPath(folderId))
                return;
    ...
    toHeader(folderId, msg) {
        const env = msg.envelope ?? {};
        const from = pickAddress(env.from?.[0]) ?? { email: '' };
        const to = (env.to ?? []).map(pickAddress).filter(Boolean);
        const cc = env.cc?.length ? env.cc.map(pickAddress).filter(Boolean) : undefined;
 ...
 */
function asFetched(msg) {
    const m = msg;
    return {
        uid: m.uid,
        envelope: m.envelope,
        flags: m.flags instanceof Set ? Array.from(m.flags) : m.flags,
