 ...
 */
export async function parseRfc822(source: Buffer): Promise<ParsedMessage> {
  ...
  if (boundary) {
    ...
    for (const part of parts) {
      ...
      if (pmime === 'text/html' && !html) html = decoded.toString('utf8');
      else if (pmime === 'text/plain' && !text) text = decoded.toString('utf8');
    }
  } else {
    const decoded = decodePart(body, headers['content-transfer-encoding'] ?? '7bit', charset);
    if (mime === 'text/html') html = decoded.toString('utf8');
...
function splitHeader(raw: string): { headerBlock: string; body: string } {
...
function parseContentType(ct: string): { mime: string; boundary?: string; charset?: string } {
...
function decodePart(body: string, encoding: string, _charset?: string): Buffer {
