...
export class OllamaSidecar {
  ...
  private reusingExternal = false;
  private startedAt = 0;
  private lastError: string | undefined;

  status(): { running: boolean; reusingExternal: boolean; uptimeMs: number; lastError?: string } {
    return {
  ...
  async start(opts: { timeoutMs?: number } = {}): Promise<void> {
    ...
    this.startedAt = Date.now();
    this.child.stdout?.on('data', (b: Buffer) => log.info('[ollama]', b.toString().trim()));
    this.child.stderr?.on('data', (b: Buffer) => {
      ...
      if (/panic|error|fatal/i.test(msg)) log.warn('[ollama]', msg);
      else log.info('[ollama]', msg);
    });
