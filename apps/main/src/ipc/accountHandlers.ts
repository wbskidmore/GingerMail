...
import type { Account, AddAccountInput } from '@gingermail/core';
import { ImapSmtpProvider, type ImapSmtpCredentials } from '@gingermail/providers';
...
export async function handleAccountAdd(
  ...
  if (input.kind === 'imap-smtp' || input.kind === 'apple-caldav') {
    ...
    if (opts.testOnly) return account;
...
function imapSmtpDefaults(input: AddAccountInput): Record<string, unknown> {
