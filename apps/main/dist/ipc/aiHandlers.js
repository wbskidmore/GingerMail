...
import { buildAiClient, buildNlSearchSpec, draftReply, extractActionItems, prioritizeInbox, summarizeThread, } from '@gingermail/ai';
/**
 ...
 */
function effectiveAiSettings(ctx) {
    const ai = ctx.getSettings().ai;
    if (ai.mode !== 'cloud' || !ai.cloud)
        return ai;
    const key = ctx.vault.readAppSecret('aiCloudApiKey');
    return { ...ai, cloud: { ...ai.cloud, apiKey: key ?? '' } };
}
export function handleAi(ctx) {
    handle(IPC_CHANNELS.aiSummarize, async (_e, threadId) => {
        const client = buildAiClient(effectiveAiSettings(ctx));
        if (!client)
            throw new Error('AI is off. Configure it in Settings.');
        const messages = ctx.db.listMessages({ threadId });
        const thread = synthesisThread(messages, threadId);
        const fullMessages = messages.map((m) => ctx.db.getMessage(m.id)).filter(Boolean);
        return summarizeThread(client, thread, fullMessages);
    });
    handle(IPC_CHANNELS.aiDraftReply, async (_e, input) => {
        const client = buildAiClient(effectiveAiSettings(ctx));
        if (!client)
            throw new Error('AI is off. Configure it in Settings.');
    ...
    handle(IPC_CHANNELS.aiExtractActions, async (_e, input) => {
        const client = buildAiClient(effectiveAiSettings(ctx));
        if (!client)
            throw new Error('AI is off. Configure it in Settings.');
        ...
        if (input.messageId) {
            ...
            const tasks = await extractActionItems(client, message, listId);
            ctx.db.upsertTasks(tasks);
            return tasks;
        }
        if (input.threadId) {
            const heads = ctx.db.listMessages({ threadId: input.threadId, limit: 20 });
            const combined = heads.map((h) => ctx.db.getMessage(h.id)).filter(Boolean);
            const tasks = [];
    ...
    handle(IPC_CHANNELS.aiNlSearch, async (_e, query) => {
        ...
        try {
            ...
            const messages = ctx.db.searchMessagesAdvanced({ ftsQuery: spec.ftsQuery, after, before, unread: spec.unread }, 100);
            return {
                messages,
                usedAi: true,
    ...
    handle(IPC_CHANNELS.aiTest, async () => {
        const client = buildAiClient(effectiveAiSettings(ctx));
        if (!client)
            return { ok: false, error: 'AI mode is off' };
        return client.testConnection();
    });
    // ---- Local AI / Ollama sidecar ----
    handle(IPC_CHANNELS.aiLocalStatus, async () => {
    ...
    handle(IPC_CHANNELS.aiListAvailableModels, async () => {
        return CURATED_MODELS.map((m) => ({ ...m }));
    });
    handle(IPC_CHANNELS.aiListInstalledModels, async () => {
        try {
    ...
    safeHandle(IPC_CHANNELS.aiPullModel, AiPullModelSchema, async (input) => {
        ...
        try {
            await client.pullModel(input.name, ({ status, completed, total }) => send({ name: input.name, status, completed, total }));
    ...
    handle(IPC_CHANNELS.aiGetCloudKeyStatus, async () => {
        const key = ctx.vault.readAppSecret('aiCloudApiKey');
        if (!key)
...
function synthesisThread(messages, threadId) {
    return {
        id: threadId,
